...
class TimeAgo {
  ...
  chatMessageTransform(value: string | number | Date) {
    const date = typeof value === 'string' ? new Date(value) : value;
  ...
  dayMonthYear(value: string | number | Date) {
    const date = typeof value === 'string' ? new Date(value) : value;
    return format(date, 'd MMMM yyyy');
  }

  timeFormat(value: string | number | Date) {
    const date = typeof value === 'string' ? new Date(value) : value;
    return format(date, 'HH:mm a');
  }

  timeDifference(current: number | Date, date: number | Date) {
  ...
  monthsAgo(date: any, elapsed: number, msPerDay: number) {
